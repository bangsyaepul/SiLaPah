...
async function handleOfficerRegister(e) {
  ...
  AppState.pendingOfficerData = { name, username, password };
...
async function handleOTPVerify() {
  ...
  const { name, username, password } = AppState.pendingOfficerData;
...
async function handleOfficerLogin(e) {
  ...
  try {
    ...
    AppState.currentUser = { uid: username, name: officer.name, username, role: 'officer' };
...
function startUserApp() {
  ...
  loadUserDashboard();
  startUserReportListener();
}

// ─── Logout ───────────────────────────────────────────────────
function logout() {

  // hapus notif navigasi
...
function startRealtimeListener() {
  ...
  reportsRef.on('value', snap => {
    ...
    Object.keys(data).forEach(id => {
      if (!lastKnownReports[id]) {
        if (Object.keys(lastKnownReports).length > 0) {
          const r = data[id];
...
function startUserReportListener() {
  ...
  reportsRef.on('value', snap => {
    ...
    Object.entries(data).forEach(([id, r]) => {
      ...
      const old = oldReports[id] || {};

      // ===================================
      // PETUGAS MENUJU LOKASI
      // ===================================
      if (
        old.officerNavigating !==
        r.officerNavigating
      ) {

        if (
          r.officerNavigating === true &&
          currentPage &&
      ...
      if (
        ...
        if (
          ...
          if (
            !sessionStorage.getItem(arrivedKey)
          ) {
    ...
    // simpan data lama
    oldReports = JSON.parse(
      JSON.stringify(data)
    );
  });

  // =======================================
// REALTIME POSISI PETUGAS UNTUK WARGA
// =======================================
window.APP_CONFIG.db
  ...
  .on('value', snap => {
    ...
    // laporan yg sedang dibuka warga
    const activeReportId =
      AppState.activeUserReportId;

    if (!activeReportId) return;

    const report =
      AppState.reports[activeReportId];

    if (!report) return;

    // cari petugas yg menangani laporan ini
    const matchedOfficer =
      Object.values(officers).find(o =>

        o.reportId === activeReportId &&

        o.category === report.category
      );

    if (
      matchedOfficer &&
      AppState.userMapInstance
    ) {

      updateOfficerMarkerOnUserMap({
        lat: matchedOfficer.lat,
        lng: matchedOfficer.lng
      });

      // tampilkan nama petugas
      $('user-map-status').textContent =
        `🚛 ${matchedOfficer.officerName} sedang menuju lokasi`;
