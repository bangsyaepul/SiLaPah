...
function showToast(msg, type = 'info', dur = 3500) {
  ...
  const icons = { success: '<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>', error: '❌', info: '<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>', warning: '⚠️' };
...
function addNotification(title, body, reportId = null) {
  ...
  showToast(`<svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:2px"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg> ${title}: ${body}`, 'info', 4000);
}
...
function renderNotifications() {
  ...
  if (AppState.notifications.length === 0) {
    list.innerHTML = `<div class="empty-state"><div class="empty-icon"><svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg></div><p class="empty-desc">Belum ada notifikasi</p></div>`;
    return;
  }
...
function toggleNotifPanel() {
  ...
  if (panel.classList.contains('open')) {
    // Apply officer or user mode for theming
    const isOfficer = AppState.currentPage && AppState.currentPage.includes('officer');
    panel.classList.toggle('notif-mode-officer', isOfficer);
    panel.classList.toggle('notif-mode-user', !isOfficer);
    renderNotifications();
  }
}
...
function updateNavigation() {
  ...
  // Jarak garis lurus (haversine) — HANYA dipakai untuk deteksi "hampir tiba",
  // BUKAN untuk ditampilkan ke panel ETA/jarak.
  // CATATAN BUG LAMA: dulu fungsi ini juga menulis ke #nav-distance & #nav-eta
  // memakai estimasi kasar (jarak lurus / asumsi 30km/h) yang rumusnya salah
  // (kelebihan kali 60) DAN berjalan tiap 10 detik lewat setInterval, sehingga
  // ETA akurat dari rute jalan (OSRM, via drawRoute) terus-menerus tertimpa
  // angka yang salah & meloncat-loncat (mis. 2 menit tiba-tiba jadi 108 menit).
  // Sekarang #nav-distance & #nav-eta HANYA diisi oleh drawRoute() (rute jalan
  // sebenarnya), jadi nilainya konsisten dan tidak ada lagi dua sumber yang
  // saling menimpa.
  ...
  // Update officer marker on map (drawRoute di dalamnya akan mengisi
  // #nav-distance & #nav-eta dengan nilai rute jalan yang akurat)
