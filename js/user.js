      ...
      <div class="report-card-meta">
        ...
        ${showOfficerInfo ? `
          <div style="display:flex;align-items:center;gap:6px;padding:5px 8px;background:rgba(78,203,141,0.07);border:1px solid rgba(78,203,141,0.18);border-radius:8px;margin-top:4px">
            <span style="font-size:14px"><svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 18h20"/><path d="M12 2a7 7 0 0 0-7 7v2h14V9a7 7 0 0 0-7-7z"/><path d="M12 2v9"/><path d="M5 9H2"/><path d="M22 9h-3"/><rect x="5" y="18" width="14" height="3" rx="1"/></svg></span>
            <div>
      ...
      <div class="modern-tracker">
        ...
        <div class="modern-tracker-steps">
          ...
          <div class="modern-step done">
            <div class="modern-step-circle"><svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="8" y="2" width="8" height="4" rx="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><line x1="9" y1="11" x2="15" y2="11"/><line x1="9" y1="15" x2="15" y2="15"/></svg></div>
          ...
          <div class="modern-step ${r.status === 'progress' || r.status === 'transporting' || r.status === 'done' ? 'done' : ''} ${r.status === 'progress' ? 'active' : ''}">
            <div class="modern-step-circle"><svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 17h4V5H2v12h3"/><path d="M14 9h4l3 4v4h-2"/><circle cx="7.5" cy="17.5" r="1.5"/><circle cx="17.5" cy="17.5" r="1.5"/></svg></div>
          ...
          <div class="modern-step ${r.status === 'transporting' || r.status === 'done' ? 'done' : ''} ${r.status === 'transporting' ? 'active' : ''}">
            <div class="modern-step-circle"><svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21V10l5-3 5 3v3l5-3v11"/><path d="M3 21h18"/><line x1="7" y1="16" x2="7" y2="16.01"/><line x1="12" y1="16" x2="12" y2="16.01"/></svg></div>
            <div class="modern-step-text">Ke TPS</div>
          </div>
          <!-- Step 4: Selesai -->
          <div class="modern-step ${r.status === 'done' ? 'done' : ''}">
            <div class="modern-step-circle"><svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/></svg></div>
...
// ─── Status Tab ───────────────────────────────────────────────
let _activeStatusFilter = 'semua'; // 'semua' | 'pending' | 'progress' | 'done'
...
function selectUserCatFilter(cat) {
  ...
  if (label) label.innerHTML = cat === 'semua' ? '🗂️ Semua Kategori' : `${CATEGORY_ICONS[cat] || '🗑️'} ${cat}`;
  const menu = document.getElementById('user-cat-dropdown-menu');
  const btn  = document.getElementById('user-cat-dropdown-btn');
...
function renderFilteredUserReports() {
  ...
  // Filter status: semua | belum dibersihkan (pending) | sedang dibersihkan (progress+transporting) | selesai (done)
  if (_activeStatusFilter && _activeStatusFilter !== 'semua') {
    if (_activeStatusFilter === 'pending') {
      reports = reports.filter(r => !r.status || r.status === 'pending');
    } else if (_activeStatusFilter === 'progress') {
      reports = reports.filter(r => r.status === 'progress' || r.status === 'transporting');
    } else if (_activeStatusFilter === 'done') {
  ...
  if (reports.length === 0) {
    let icon = '📸';
    if (_activeCatFilter !== 'semua') icon = CATEGORY_ICONS[_activeCatFilter] || '🗑️';
    else if (_activeStatusFilter === 'pending')  icon = '';
    else if (_activeStatusFilter === 'progress') icon = '';
    else if (_activeStatusFilter === 'done')     icon = '✅';
    container.innerHTML = `
